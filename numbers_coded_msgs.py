...
"""
Failed Trial: Prasanth
-----------------------------------
traversed_memo = []
success_memo = []

def memoize_subseq_sum(a_list,t_sum, start_idx, end_idx):
    while start_idx <= end_idx and t_sum >= 0:
        if (start_idx, end_idx, t_sum) not in traversed_memo:
            result_subseq_sum =  memoize_subseq_sum(a_list[1:],t_sum,start_idx + 1, end_idx) || memoize_subseq_sum(a_list[1:],t_sum - a_list[0] ,start_idx , end_idx) || memoize_subseq_sum(a_list[:-1],t_sum, start_idx, end_idx -1) || memoize_subseq_sum(a_list[:-1],t_sum - a_list[-1], start_idx , end_idx)
            if result_subseq_sum :
                success_memo.append()
                return result_subseq_sum


def answer(l,t):
    beg_idx = 0
    term_idx = len(l)-1
    test_for_subseq_sum(l,t, beg_idx,term_idx)
